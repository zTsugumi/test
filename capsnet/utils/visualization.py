...
#   _, axes = plt.subplots(r, c, figsize=(10, 10))
#   axes = axes.flatten()
...
def plot_image(r, c, imgs, vmin=0., vmax=1., figsize=(5, 5)):
  if r == 1 and c == 1:
    plt.imshow(imgs, cmap='gray', norm=None, vmin=vmin, vmax=vmax)
    plt.show()
  else:
    ...
    for i, ax in zip(imgs, axes):
      ax.set_xticks([])
      ax.set_yticks([])
      ax.imshow(i, cmap='gray', norm=None, vmin=vmin, vmax=vmax)
